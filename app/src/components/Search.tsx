...
export const Search = ({ placeholder, onSearchChange }: { placeholder: string, onSearchChange: (searchTerm: string) => void }) => {
    ...
    return (
        <form className="flex items-center relative sm:w-96 w-80 mr-4">
            <div className="relative w-96">
                <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
                    <img src={searchicon} className="w-5 h-5" alt="Search icon" />
                </div>
                <input className="block sm:w-96 w-80 p-4 pl-12 text-white bg-[#222] border-[#333] rounded-xl focus:border-indigo-400 hover:border-indigo-400"
                    type="text"
                    value={search}
